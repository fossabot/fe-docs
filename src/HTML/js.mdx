...
若为new关键字实例出来的对象， this则一直指向当前这个对象
在事件中， this指向触发这个事件的对象（IE中的attachEvent中的this总指向全局对象window）
```
...
radix(可选): 从2～36， 代表该进位系统的数字。例如说指定10即为10进位。当没有指定或为0时按照10为基数来解析。

若string无法被转换为string或radix不在2～36之内则解析为NaN
